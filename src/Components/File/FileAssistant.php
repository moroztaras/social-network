...
use App\Components\File\Events\FilePrivateEvent;
...
class FileAssistant
{
    ...
    public function __construct(CurrentUser $currentUser, RequestStack $requestStack, Filesystem $filesystem, RouterInterface $router, EventDispatcherInterface $dispatcher)
    {
    ...
    public function webDir($url, $absolute = false)
    {
        ...
        if ($absolute) {
            ...
            $url = $context->getScheme().'://'.$context->getHost().$url;
        }

        return $url;
    }

    public function webUrl(File $file)
    {
    }
    ...
    {
        return $this->isPrivateUrl($file->getUrl());
    }

    public function isPrivateUrl($url)
    {
        return false !== strpos($url, 'private://');
    }
    ...
    public function prepareUploadFile(UploadedFile $upload_file, $folder, $private = false)
    {
        ...
        $file->setFilename($fileName)->setFileSize($upload_file->getSize());

        $file->setUser($this->currentUser->getUserId());
    ...
    public function getFolderDay()
    {
        ...
        return date('j', $date->getTimestamp());
    }

    public function outputVideo($link, array $options = [])
    {
        return sprintf('<video src="%s?t=%s" controls controlsList="nodownload"></video>', $link, time());
    }

    public function privateFileAccess(File $file)
    {
        if ($file->getHandler()) {
            $filePrivateEvent = new FilePrivateEvent();
            $filePrivateEvent->setFile($file);
            $this->dispatcher->dispatch(FileEvents::PRIVATE_READER_ACCESS, $filePrivateEvent);

            return $filePrivateEvent->getAccess();
        }
